<x-app-layout>
    <div class="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop py-xl">
        <div class="mb-lg flex items-center justify-between">
            <h1 class="text-headline-lg font-headline-lg text-on-surface">Đăng tin mới</h1>
        </div>
        ...
        <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data" class="bg-surface-container-lowest rounded-2xl shadow-sm border border-outline-variant overflow-hidden" x-data="imageUploader()">
            @csrf
                ...
                <section>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-md">
                        <div class="col-span-full">
                            ...
                            <input type="text" name="title" value="{{ old('title') }}" required placeholder="VD: iPhone 14 Pro Max 256GB Chính hãng" class="w-full rounded-lg border-outline-variant bg-surface focus:ring-primary focus:border-primary shadow-sm text-body-md text-on-surface">
                        </div>

                        <div>
                            <label class="block text-label-md font-label-md text-on-surface mb-xs">Danh mục <span class="text-error">*</span></label>
                            <x-custom-select name="category_id" :options="$categoryOptions" :selected="old('category_id')" placeholder="Chọn danh mục" :required="true" />
                        </div>

                        <div>
                            <label class="block text-label-md font-label-md text-on-surface mb-xs">Giá bán (VNĐ) <span class="text-error">*</span></label>
                            <div class="relative">
                                <input type="number" name="price" value="{{ old('price') }}" required min="1000" step="1000" class="w-full pl-4 pr-12 rounded-lg border-outline-variant bg-surface focus:ring-primary focus:border-primary shadow-sm text-body-md text-on-surface">
                        ...
                        <div>
                            ...
                            <input type="number" name="quantity" value="{{ old('quantity', 1) }}" required min="1" class="w-full rounded-lg border-outline-variant bg-surface focus:ring-primary focus:border-primary shadow-sm text-body-md text-on-surface">
                        </div>

                        <div>
                            <label class="block text-label-md font-label-md text-on-surface mb-xs">Tình trạng (%) <span class="text-error">*</span></label>
                            <input type="number" name="condition_percent" value="{{ old('condition_percent', 95) }}" required min="1" max="100" class="w-full rounded-lg border-outline-variant bg-surface focus:ring-primary focus:border-primary shadow-sm text-body-md text-on-surface">
                        ...
                        <div class="flex items-center pt-md">
                            ...
                            <input type="checkbox" name="is_negotiable" value="1" id="is_negotiable" {{ old('is_negotiable') ? 'checked' : '' }} class="rounded border-outline-variant text-primary shadow-sm focus:ring-primary bg-surface">
                ...
                <section>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-md mb-md">
                        <div>
                            ...
                            <input type="text" name="brand" value="{{ old('brand') }}" placeholder="VD: Apple, Samsung" class="w-full rounded-lg border-outline-variant bg-surface focus:ring-primary focus:border-primary shadow-sm text-body-md text-on-surface">
                        </div>
                        <div>
                            <label class="block text-label-md font-label-md text-on-surface mb-xs">Dòng sản phẩm</label>
                            <input type="text" name="model" value="{{ old('model') }}" placeholder="VD: iPhone 14 Pro Max" class="w-full rounded-lg border-outline-variant bg-surface focus:ring-primary focus:border-primary shadow-sm text-body-md text-on-surface">
                        </div>
                    </div>

                    <div>
                        <label class="block text-label-md font-label-md text-on-surface mb-xs">Mô tả chi tiết <span class="text-error">*</span></label>
                        <textarea name="description" rows="6" required placeholder="Mô tả chi tiết tình trạng, phụ kiện đi kèm, thời gian bảo hành..." class="w-full rounded-lg border-outline-variant bg-surface focus:ring-primary focus:border-primary shadow-sm text-body-md text-on-surface">{{ old('description') }}</textarea>
                    </div>
                </section>
                ...
                <section>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-md mb-md">
                        <div>
                            ...
                            <x-custom-select name="province_id" :options="$provinceOptions" :selected="old('province_id')" placeholder="Chọn khu vực" :required="true" />
                        </div>
                        <div>
                    ...
                    <!-- Image Upload -->
                    <div>
                        <label class="block text-label-md font-label-md text-on-surface mb-xs">Hình ảnh sản phẩm (tối đa 5 ảnh) <span class="text-error">*</span></label>
                        ...
                        >
                            <label :class="isDragging ? 'border-primary bg-primary-container' : 'border-outline-variant bg-surface-container-lowest hover:bg-surface-container-low'" class="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed rounded-xl cursor-pointer transition-colors">
                                ...
                                <input type="file" id="images_input" name="images[]" multiple accept="image/*" class="hidden" @change="handleFileInput($event)">
                            </label>
                        </div>
                        ...
                        <div class="mt-md grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-sm" x-show="images.length > 0" style="display: none;">
                            <template x-for="(image, index) in images" :key="index">
                                <div class="relative aspect-square rounded-lg overflow-hidden border border-outline-variant group">
                                    ...
                                    <div class="absolute top-1 left-1 bg-primary text-on-primary text-[10px] font-bold px-1.5 py-0.5 rounded" x-show="index === 0">Ảnh bìa</div>
